WITH quote_dedup AS (
    ...
    QUALIFY ROW_NUMBER() OVER (
        ...
        ORDER BY quote_date DESC NULLS LAST, quote_number DESC
    ) = 1
...
edh_opps AS (
    ...
    SELECT
        ...
        edh_opp.projected_close_date::DATE                              AS opportunity_projected_close_date,
        edh_line.quantity,
    ...
    WHERE acct.account_type IN ('End Customer', 'Strategic Account', 'Government', 'A.D.N.')
      ...
      AND edh_opp.projected_close_date::DATE BETWEEN '2026-02-01'::DATE AND '2028-01-31'::DATE
...
acs_opps AS (
    ...
    SELECT
        ...
        acs_opp.close_date::DATE                                        AS opportunity_projected_close_date,
        acs_line.quantity,
    ...
    WHERE edh_opp.opportunity_number IS NULL
      AND NOT (geo.country IS NOT NULL AND acs_opp.is_closed = FALSE)
      ...
      AND cust_acct.is_visible = TRUE
      AND COALESCE(cust_acct.account_category, '') NOT IN ('Internal', 'Partner')
      AND acs_opp.close_date::DATE BETWEEN '2026-02-01'::DATE AND '2028-01-31'::DATE
